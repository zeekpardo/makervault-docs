...
| **Embedded Thumbnail** | `<Thumbnail Source>` | PNG preview mode |
| **Shapes & Layers** | `<Shape>` and `<CutSetting>` elements | Interactive canvas preview |
| **Material Height (Z offset)** | `zOffset` on cut settings | Detail panel (when non-zero) |
| **Kerf** | `kerf` on cut settings | Cut settings table, per layer |
| **Shape Breakdown** | `<Shape>` elements by type | Detail panel (paths, text, bitmaps) |
